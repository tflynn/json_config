...
class JsonConfig:
    ...
    @classmethod
    def conf(cls, logger=None, package_name=None, package_dir=None):
        """
        ...
        if not cls.COMMON_CONFIG:
            cls.COMMON_CONFIG = JsonConfig(logger=logger, package_name=package_name, package_dir=package_dir)
        return cls.COMMON_CONFIG

    @classmethod
    def new_conf(cls, logger=None, package_name=None, package_dir=None):
        """
        ...
        :rtype: Config
        """
        return JsonConfig(logger=logger, package_name=package_name, package_dir=package_dir)
    ...
    def get_data(self):
        ...
        """
        if self.is_loaded():
            self.logger.debug("JsonConfig.get_data data is already loaded")
        else:
            self.logger.debug("JsonConfig.get_data data is not yet loaded so load it")
            self.load_all_data()
    ...
    def absolute_package_location(self, package_relative_path):
        ...
        :param str package_relative_path: Package relative path e.g. <package>/conf.json
    ...
    def expand_search_paths(self):
        ...
        # Only load once
        if self.is_loaded():
            self.logger.debug("JsonConfig.expand_search_paths data is already loaded, so paths have already been expanded. Don't do it again")
            return self.full_search_paths

        # Only expand once
        if self.are_paths_expanded():
            self.logger.debug("JsonConfig.expand_search_paths have already been expanded, so don't do it again")
            return self.full_search_paths
        ...
        self.logger.debug("JsonConfig.expand_search_paths search paths {0}".format(self.search_paths))
        ...
        self.logger.debug("JsonConfig.expand_search_paths full search paths {0}".format(self.full_search_paths))
    ...
    def load_all_data(self):
        ...
        # Only load once
        if self.is_loaded():
            self.logger.debug("JsonConfig.load_all_data data has been loaded, so don't do it again")
            return self.conf_data
        ...
        self.loaded = True
        self.logger.debug("JsonConfig.load_all_data loaded search paths {0}".format(self.loaded_search_paths))
        self.logger.debug("JsonConfig.load_all_data data has been loaded {0}".format(self.conf_data))
        return self.conf_data
